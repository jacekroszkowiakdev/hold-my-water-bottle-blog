...
if [ -f /var/www/html/wordpress/wp-config-sample.php ]; then
  ...
  sudo echo "define('WP_SITEURL', 'http://$domain_name/wordpress');" | sudo tee -a /var/www/html/wordpress/wp-config.php
  sudo echo "define('WP_HOME', 'http://$domain_name');" | sudo tee -a /var/www/html/wordpress/wp-config.php
...
echo sudo amazon-linux-extras install epel -y >> /home/ec2-user/stress_test.sh
echo sudo yum install -y stress >> /home/ec2-user/stress_test.sh
echo "Starting stress test..." >> /home/ec2-user/stress_test.sh
echo stress --cpu 8 --timeout 800 >> /home/ec2-user/stress_test.sh
